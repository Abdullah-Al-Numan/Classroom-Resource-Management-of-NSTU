...
router.get('/allpost/:dep_session', requireLogin, (req, res) => {
    console.log(req.originalUrl);
    console.log(req.url);
    console.log(req.params);
    console.log(req.params.dep_session);



    Post.find({dep_session:`/${req.params.dep_session}`})
...
router.post('/createpost', requireLogin, (req, res) => {
    const { title, body, picUrl, dep_session } = req.body;
    if (!title || !body || !picUrl || !dep_session) {
        return res.json({ error: 'Please add all the fields!' });
    }
    const post = new Post({ title, body, photo: picUrl, dep_session, postedBy: req.user });
    post.save()
        .then(post => {
