...
const Home = () => {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`/solverallpost${window.location.pathname}`, {
            headers: {
    ...
    const deletePost = (postId) => {
        axios.delete(`/solverdeletepost/${postId}`, {
            headers: {
                "Authorization": "Bearer " + localStorage.getItem("jwt")
            }
        })
    ...
    const deleteComment = (postId, commentId) => {
        axios.delete(`/solverdeletecomment/${postId}/${commentId}`, {
            headers: {
                "Authorization": "Bearer " + localStorage.getItem("jwt")
            }
        })
    ...
    const likedUsers = (postId) => {
        ...
        axios.post('/solverlikedusers', { postId }, {
            headers: {
                "Content-Type": "application/json",
                "Authorization": "Bearer " + localStorage.getItem("jwt")
            }
                ...
                data.map(item => (
                    <div className="card home-card" key={item._id}>
                        ...
                        <div className="card-content">
                            ...
                            <h6 data-target="modalLikes" className="modal-trigger ml-3" style={{ cursor: "pointer" }} onClick={() => likedUsers(item._id)} >{item.likes.length} </h6>
                            </div>
                            <h6>{item.title}</h6>
                            <p>{item.body}</p>
                            {
                            ...
                            <form onSubmit={(event) => {
                                ...
                                <TextField
                                 ...
                                 multiline
                              />
                            <button type='submit' className="btn waves-effect waves-light #01579b light-blue darken-4 white-text text-darken-2 mt-1">submit</button>
                            </form>
